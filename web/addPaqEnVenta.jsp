        ...
        <div class="page-wrapper bg-gra-02 p-t-130 p-b-100 font-poppins">
            <div class="wrapper wrapper--w680">
                <div class="card card-4">
                    <div class="card-body">
                        <h2 class="title">Añadir Paquete</h2>
                        <form action = "SvAddPaqEnVenta" method="POST">
                            <%String id = request.getAttribute("id").toString(); %>
                            <input type="hidden" name="id" value="<%=id%>">                            
                            <div class="row row-space">
                                <div class="col-2">
                                    <div class="input-group">
                                        <label class="label">Paquetes Disponibles</label>
                                        <select class="input--style-4" type="combobox" name="paquete">
                                            <% Controladora control = new Controladora();
                                                 List<Paquete_turistico> listaP = control.traerPaquetes();
                                            for (Paquete_turistico paq : listaP) 
                                            { %>
                                                <option value="<%=paq.getCodigo_paquete()%>"><%=paq.getNombre()%></option>
                                            <%}%>                                           
                                        </select>
                                    </div>
                                </div>
